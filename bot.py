...
class InfoView(View):
    ...
    async def accept(self, interaction: Interaction, button: Button):
        ...
        # Professional Hitting Tutorial Embed
        embed = Embed(
...
async def stats(interaction: Interaction, user: discord.Member):
    ...
    description = ""
    for v in data["vouches"][-10:]: # Show last 10
...
async def manageban(interaction: Interaction, user: str, action: str):
    ...
    try:
        # Extract ID from mention or use raw string
        ...
        if action.lower() == "ban":
            # Attempt to fetch member/user to get a name for the log
            try:
